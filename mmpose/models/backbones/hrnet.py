...
class HRModule(BaseModule):
    ...
    def Dupsampling(self,x,scale):
        ...
        # N, H, W*scale, C/scale
        x_permuted = x_permuted.contiguous().view((N, H, W * scale, int(C / (scale))))

        # N, W*scale,H, C/scale
        x_permuted = x_permuted.permute(0, 2, 1, 3)
        # N, W*scale,H*scale, C/(scale**2)
        x_permuted = x_permuted.contiguous().view((N, W * scale, H * scale, int(C / (scale * scale))))
    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_out_branches):
            ...
            for j in range(num_branches):
                if j > i:
                    # out_channeli=in_channels[i]*((2**(j - i))**2)
                    fuse_layer.append(
                        nn.Sequential(
                            build_conv_layer(
                                self.conv_cfg,
                                in_channels[j]//((2**(j - i))**2),
                                # in_channels[j],
                                # out_channeli,
                                in_channels[i],
                                kernel_size=1,
                                ...
                                padding=0,
                                bias=False),
                            # build_norm_layer(self.norm_cfg, out_channeli)[1],
                            build_norm_layer(self.norm_cfg, in_channels[i])[1],#in_channels[i]
                            # nn.Upsample(
                            #     scale_factor=2**(j - i),
    ...
    def forward(self, x):
        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(self.num_branches):
                if i == j:
                    y += x[j]
                elif j > i:#i=0,j=1,2;i=1,j=2
                    temp = self.pixel_shuffle(x[j], 2**(j - i))
                    y += self.fuse_layers[i][j](temp)
                    # temp = self.fuse_layers[i][j](x[j])
                    # y += self.Dupsampling(temp, 2**(j - i))
                    # temp = self.fuse_layers[i][j](x[j])
                    # y += self.pixel_shuffle(temp, 2**(j - i))
                else:
