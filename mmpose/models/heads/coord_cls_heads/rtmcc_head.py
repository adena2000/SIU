...
class RTMCCHead(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        """
        feats = feats[-1]
        print(f'feats size begin is {feats.shape}')
        feats = self.final_layer(feats)  # -> B, K, H, W
        print(f'feats size now is {feats.shape}') 
        ...
        print(f'pred_x is{pred_x.shape}')
        print(f'pred_y is{pred_y.shape}')
#         feats size begin is torch.Size([64, 768, 8, 6])
# feats size now is torch.Size([64, 133, 8, 6])
# pred_x istorch.Size([64, 133, 384])
# pred_y istorch.Size([64, 133, 512])
