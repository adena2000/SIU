...
def inference_topdown(model: nn.Module,
    ...
    for bbox in bboxes:
        ...
        data_list.append(pipeline(data_info))#getbboxcenterscale

    if data_list:
        # collate data list into a batch, which is a dict with following keys:
        # batch['inputs']: a list of input images
        # batch['data_samples']: a list of :obj:`PoseDataSample`
        batch = pseudo_collate(data_list)#[2,6,..]2是input和data_samples
        with torch.no_grad():
            results = model.test_step(batch)#pred_x istorch.Size([6, 133, 576])
                                            #pred_y istorch.Size([6, 133, 768])
    else:
        results = []
